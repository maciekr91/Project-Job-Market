...
def load_from_db():
    ...
    with sqlite3.connect(DB_PATH) as connection:
        ...
        db_df['added_at'] = pd.to_datetime(db_df['added_at'], format='%Y-%m-%d %H:%M')
...
def update_voivodeship(updated_df: pd.DataFrame):
    ...
    filtered_df = updated_df[updated_df['voivodeship'] != 'Not specified']
    update_data = list(zip(filtered_df['voivodeship'], filtered_df['id']))
